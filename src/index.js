...
const getWithMrr = (GG, macros, dataTypes) => (mrrStructure, render = null, parentClass = null) => {
    ...
    const cls = class Mrr extends parent {
        constructor(props, context, already_inited, isGG) {
            ...
            this.mrr = new MrrCore(props, context, already_inited, isGG, (real_update, cb, sth) => {
                return (parent.prototype.setState || (() => {})).call(this, real_update, cb, sth);
            }, this.getMrrStruct(), macros, GG, dataTypes);
                    ...
                    props['on' + event_type] = e => {
                        for(let [selector, handler, cell] of self.mrr.__mrr.root_el_handlers[event_type]){
                            if(e.target.matches('#__mrr_root_node_n' + self.mrr.__mrr.id + ' ' + selector)){
                                ...
                                self.mrr.setState({[cell]: value}, null, null, true);
                            }
                        }
                    }
                }
...
const initGlobalGrid = (struct, availableMacros, availableDataTypes) => {
    const GG = new MrrCore({}, null, false, true, () => {}, struct, availableMacros, null, availableDataTypes, true);
